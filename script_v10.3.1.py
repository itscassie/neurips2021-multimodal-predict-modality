...
if input_train_mod2.var['feature_types'][0] == 'ATAC':
    ...
    if LOAD_MODEL:
        # model (1) pca
        y1_pred_pca = pca(input_train_mod1, input_test_mod1, n=50, alg='arpack')

        # model (8b) concat
        model_pth = meta['resources_dir'] + '/model/weights_v10/gex2atac/model_best_AtoB_cycle_gex2atac_v2_tfidfconcat_Nov18-07-03.pt'
        ...
        # ensemble
        y_pred = (
            np.array(y1_pred_pca) + np.array(y8b_pred_concat)
        ) / 2
    ...
    if LOAD_MODEL:
        ...
        # model (22) pca
        y22_pred_pca = pca(input_train_mod1, input_test_mod1, n=120)

        # model (14b) batchgan
        model_pth = meta['resources_dir'] + '/model/weights_v10/gex2adt/model_best_batchgan_gex2adt_v2_Nov16-15-47.pt'
        y14b_pred_bgan = pretrin_batchgan(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, cls_num=10)
        ...
        y_pred = (
            ...
            np.array(y22_pred_pca) + np.array(y14b_pred_bgan)
        ) / 10
    ...
    if LOAD_MODEL:
        ...
        # model (2) concat
        model_pth = meta['resources_dir'] + '/model/weights_v10/adt2gex/model_AtoB_cycle_adt2gex_concat_.pt'
        ...
        # model (3) concat
        model_pth = meta['resources_dir'] + '/model/weights_v10/adt2gex/model_best_AtoB_cycle_adt2gex_tfidfconcat_Nov03-14-47.pt'
        ...
        # ensemble
        y_pred = (
            np.array(y1_pred_pca) + np.array(y2_pred_concat) + np.array(y3_pred_concat)
            ) / 3
    ...
    if LOAD_MODEL:
        # model (5b) ga
        model_pth = meta['resources_dir'] + '/model/weights_v10/atac2gex/model_best_AtoB_cycle_atac2gex_v2_ga_Nov19-00-04.pt'
        y5b_pred_ga = pretrin_nn(
            par['input_test_mod1'], model_pth, 19039, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, gene_activity=True)

        # model (6b) ga
        model_pth = meta['resources_dir'] + '/model/weights_v10/atac2gex/model_best_AtoB_cycle_atac2gex_v2_ga_Nov19-10-45.pt'
        y6b_pred_ga = pretrin_nn(
            par['input_test_mod1'], model_pth, 19039, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, gene_activity=True)

        # model (8b) tfidf
        model_pth = meta['resources_dir'] + '/model/weights_v10/atac2gex/model_best_AtoB_cycle_atac2gex_v2_tfidf_Nov16-23-08.pt'
        ...
        # model (12b) tfidfga
        model_pth = meta['resources_dir'] + '/model/weights_v10/atac2gex/model_best_AtoB_cycle_atac2gex_v2_select2pct_tfidfconcatga_new_Nov22-11-45.pt'
        ...
        # ensemble
        y_pred = (
            np.array(y5b_pred_ga) + np.array(y6b_pred_ga) + \
            np.array(y8b_pred_tfidf) + np.array(y12b_pred_tfidfga)
            ) / 4
