...
if input_train_mod2.var['feature_types'][0] == 'ATAC':
    ...
    if LOAD_MODEL:
        # model (1) pca
        y_pred_pca = pca(input_train_mod1, input_test_mod1, n=50, alg='arpack')
        # ensemble
        y_pred = np.array(y_pred_pca)
    ...
    if LOAD_MODEL:
        ...
        # model (8b) nn
        model_pth = meta['resources_dir'] + '/model/weights_v6/gex2adt/model_best_nn_gex2adt_v2_reg1_Nov18-21-24.pt'
        y8b_pred_nn = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)

        # model (9b) nn
        model_pth = meta['resources_dir'] + '/model/weights_v6/gex2adt/model_best_nn_gex2adt_v2_new_Nov20-09-35.pt'
        y9b_pred_nn = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)

        # model (11b) nn
        model_pth = meta['resources_dir'] + '/model/weights_v6/gex2adt/model_best_nn_gex2adt_v2_dropout0.3_Nov23-19-20.pt'
        y11b_pred_nn = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)

        # model (25b) batchgan
        model_pth = meta['resources_dir'] + '/model/weights_v6/gex2adt/model_best_batchgan_gex2adt_v2_dropout0.3_Nov24-22-36.pt'
        y25b_pred_bgan = pretrin_batchgan(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, cls_num=10)

        # model (16b) concat
        model_pth = meta['resources_dir'] + '/model/weights_v6/gex2adt/model_best_nn_gex2adt_v2_tfidfconcat_Nov18-14-53.pt'
        ...
        # model (17b) concat
        model_pth = meta['resources_dir'] + '/model/weights_v6/gex2adt/model_best_nn_gex2adt_v2_tfidfconcat_new_Nov20-00-53.pt'
        ...
        # model (18b) concat
        model_pth = meta['resources_dir'] + '/model/weights_v6/gex2adt/model_best_nn_gex2adt_v2_tfidfconcat_dropout0.3_Nov24-10-24.pt'
        mod1_idf = np.load(meta['resources_dir'] + '/model/idf_matrix/gex2adt/mod1_idf_v2.npy')
        ...
        # ensemble (7)
        y_pred = (
            np.array(y8b_pred_nn) + np.array(y9b_pred_nn) + np.array(y11b_pred_nn) + \
    ...
    if LOAD_MODEL:
        # model (3) concat
        model_pth = meta['resources_dir'] + '/model/weights_v6/adt2gex/model_best_AtoB_cycle_adt2gex_tfidfconcat_Nov03-14-47.pt'
        ...
        # model (8b) cycle
        model_pth = meta['resources_dir'] + '/model/weights_v6/adt2gex/model_best_AtoB_cycle_adt2gex_v2_dropout0.0_reg0_Nov22-18-51.pt'
        y8b_pred_nn = pretrin_nn(
            par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM
        )

        # model (11b) concat
        model_pth = meta['resources_dir'] + '/model/weights_v6/adt2gex/model_best_AtoB_cycle_adt2gex_v2_tfidfconcat_dropout0.0_reg0_Nov23-02-02.pt'
        mod1_idf = np.load(meta['resources_dir'] + '/model/idf_matrix/adt2gex/mod1_idf_p2.npy')
        y11b_pred_concat = pretrin_nn(
            par['input_test_mod1'], model_pth, MOD1_DIM*2, MOD2_DIM, FEAT_DIM, HIDDEN_DIM,
            tfidf=2, idf_matrix=mod1_idf
        )
        ...
        # ensemble
        y_pred = (
            np.array(y3_pred_concat) + np.array(y8b_pred_nn) + np.array(y11b_pred_concat) * 2 + np.array(y12_pred_pca)
            ) / 5
    ...
    if LOAD_MODEL:
        # model (4b) ga
        model_pth = meta['resources_dir'] + '/model/weights_v6/atac2gex/model_best_AtoB_cycle_atac2gex_v2_Nov17-16-48.pt'
        y4b_pred_ga = pretrin_nn(
            par['input_test_mod1'], model_pth, 19039, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, gene_activity=True)

        # model (6b) ga
        model_pth = meta['resources_dir'] + '/model/weights_v6/atac2gex/model_best_AtoB_cycle_atac2gex_v2_ga_Nov19-10-45.pt'
        y6b_pred_ga = pretrin_nn(
            par['input_test_mod1'], model_pth, 19039, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, gene_activity=True)
        
        # model (12b) tfidfga
        model_pth = meta['resources_dir'] + '/model/weights_v6/atac2gex/model_best_AtoB_cycle_atac2gex_v2_select2pct_tfidfconcatga_new_Nov22-11-45.pt'
        ...
        y12b_pred_tfidfga = pretrin_nn(
            par['input_test_mod1'], model_pth, 48956, MOD2_DIM, FEAT_DIM, HIDDEN_DIM,
            tfidf=3, idf_matrix=mod1_idf, mod1_idx_path=mod1_2pct_path
        )

        # ensemble 3
        y_pred = (np.array(y4b_pred_ga) + np.array(y6b_pred_ga) + np.array(y12b_pred_tfidfga)) / 3
