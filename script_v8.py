...
if input_train_mod2.var["feature_types"][0] == "ATAC":
    ...
    if LOAD_MODEL:
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/gex2atac/model_best_AtoB_cycle_gex2atac_p2_Nov25-01-13.pt"
        )
        yp1b_pred_nn = pretrin_nn(
            par["input_test_mod1"], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM
        )
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/gex2atac/model_best_AtoB_cycle_gex2atac_p2_tfidfconcat_Nov25-05-37.pt"
        )
        mod1_idf = np.load(
            meta["resources_dir"] + "/model/idf_matrix/gex2atac/mod1_idf_p2.npy"
        )
        ...
        y_pred = (
            ...
            + np.array(yp1b_pred_nn)
            + np.array(yp3b_pred_concat)
        ) / 6
    ...
    if LOAD_MODEL:
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/gex2adt/model_best_nn_gex2adt_p2_dropout0.3_Nov26-02-38.pt"
        )
        yp2b_pred_nn = pretrin_nn(
            par["input_test_mod1"], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM
        )
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/gex2adt/model_best_nn_gex2adt_p2_tfidfconcat_dropout0.4_Nov25-22-26.pt"
        )
        mod1_idf = np.load(
            meta["resources_dir"] + "/model/idf_matrix/gex2adt/mod1_idf_v2.npy"
        )
        ...
        y_pred = (
            ...
            + np.array(yp2b_pred_nn)
            + np.array(yp9b_pred_concat)
        ) / 10
    ...
    if LOAD_MODEL:
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/adt2gex/model_best_AtoB_cycle_adt2gex_p2_dropout0.0_wo2_Nov26-02-52.pt"
        )
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/adt2gex/model_best_AtoB_cycle_adt2gex_p2_tfidfconcat_dropout0.0_wo2_Nov26-04-06.pt"
        )
        mod1_idf = np.load(
        ...
        y_pred = (
            ...
            + np.array(yp5b_pred_nn)
            + np.array(yp7b_pred_concat)
        ) / 6
    ...
    if LOAD_MODEL:
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/atac2gex/model_best_AtoB_cycle_atac2gex_p2_ga_Nov24-16-20.pt"
        )
        ...
        model_pth = (
            meta["resources_dir"]
            + "/model/weights_v8/atac2gex/model_best_AtoB_cycle_atac2gex_p2_select2pct_Nov24-23-16.pt"
        )
        ...
        # ensemble
        y_pred = (
            np.array(y5b_pred_ga)
            + np.array(y6b_pred_ga)
            + np.array(y8b_pred_tfidf)
            + np.array(yp1b_pred_ga)
            + np.array(yp3b_pred_2pct)
        ) / 5
