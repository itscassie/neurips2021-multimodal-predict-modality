...
if input_train_mod2.var['feature_types'][0] == 'ATAC':
    ...
    if LOAD_MODEL:
        ...
        # ensemble
        y_pred = (np.array(y1_pred_pca) + np.array(y5b_pred_nn) + \
            np.array(y6b_pred_nn) + np.array(y8b_pred_concat)) / 4
    ...
    if LOAD_MODEL:
        ...
        # ensemble
        y_pred = (np.array(y5b_pred_ga) + np.array(y6b_pred_ga) + np.array(y8b_pred_tfidf)) / 3
