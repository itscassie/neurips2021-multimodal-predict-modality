...
if input_train_mod2.var['feature_types'][0] == 'ATAC':
    ...
    if LOAD_MODEL:
        ...
        # model (2) nn
        model_pth = meta['resources_dir'] + '/model/weights_v4/gex2atac/model_best_AtoB_cycle_gex2atac_v2_Nov17-23-18.pt'
        y_pred_nn = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)
        

        # model (3) concat
        model_pth = meta['resources_dir'] + '/model/weights_v4/gex2atac/model_best_AtoB_cycle_gex2atac_v2_tfidfconcat_Nov18-07-03.pt'
        ...
        # ensemble
        y_pred = (np.array(y_pred_pca) + np.array(y_pred_nn) + np.array(y_pred_concat)) / 3
    ...
    if LOAD_MODEL:
        ...
        # model (6) nn
        model_pth = meta['resources_dir'] + '/model/weights_v4/gex2adt/model_best_nn_gex2adt_v2_Nov16-16-02.pt'
        y_pred_nn = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)
        
        # model (7) nn 2
        model_pth = meta['resources_dir'] + '/model/weights_v4/gex2adt/model_best_nn_gex2adt_v2_reg1_Nov18-21-24.pt'
        y_pred_nn2 = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)
        
        # model (8) bgan
        model_pth = meta['resources_dir'] + '/model/weights_v4/gex2adt/model_best_batchgan_gex2adt_v2_Nov16-15-47.pt'
        y_pred_bgan = pretrin_batchgan(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, cls_num=10)

        # model (10) concat
        model_pth = meta['resources_dir'] + '/model/weights_v4/gex2adt/model_best_nn_gex2adt_v2_tfidfconcat_Nov18-14-53.pt'
        ...
        # model (19) concat 2
        model_pth = meta['resources_dir'] + '/model/weights_v4/gex2adt/model_nn_gex2adt_v2_tfidfconcat_Nov17-13-24.pt'
        ...
        # (test v1) use best
        y_pred = (
            np.array(y_pred_nn) + np.array(y_pred_nn2) + np.array(y_pred_bgan) + \
            np.array(y_pred_concat) + np.array(y_pred_pca) + np.array(y_pred_concat2)
        ) / 6
    ...
    if LOAD_MODEL:
        ...
        # model (2) concat
        model_pth = meta['resources_dir'] + '/model/weights_v4/adt2gex/model_AtoB_cycle_adt2gex_concat_.pt'
        ...
        # model (3) concat 2
        model_pth = meta['resources_dir'] + '/model/weights_v4/adt2gex/model_best_AtoB_cycle_adt2gex_tfidfconcat_Nov03-14-47.pt'
        y_pred_concat2 = pretrin_nn(
            par['input_test_mod1'], model_pth, MOD1_DIM*2, MOD2_DIM, FEAT_DIM, HIDDEN_DIM,
            tfidf=2, idf_matrix=mod1_idf
        )
        ...
        # model (5) concat 3
        model_pth = meta['resources_dir'] + '/model/weights_v4/adt2gex/model_best_AtoB_cycle_adt2gex_v2_tfidfconcat_Nov17-21-42.pt'
        ...
        # model (6) nn
        model_pth = meta['resources_dir'] + '/model/weights_v4/adt2gex/model_best_AtoB_cycle_adt2gex_v2_Nov17-18-23.pt'
        ...
        # (test_v1) v1 + v2 model
        y_pred = (
            np.array(y_pred_pca) + np.array(y_pred_concat) + np.array(y_pred_concat2) + \
            np.array(y_pred_concat3) + np.array(y_pred_nn)
            ) / 5
        # (test_v2) v2 model
        # y_pred = (
    ...
    if LOAD_MODEL:
        # model (4) ga
        model_pth = meta['resources_dir'] + '/model/weights_v4/atac2gex/model_AtoB_cycle_atac2gex_v2_ga_Nov19-00-04.pt'
        y_pred_ga = pretrin_nn(
            par['input_test_mod1'], model_pth, 19039, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, gene_activity=True)

        # model (11) ga 2
        model_pth = meta['resources_dir'] + '/model/weights_v4/atac2gex/model_AtoB_cycle_atac2gex_v2_ga_Nov19-10-45.pt'
        y_pred_ga2 = pretrin_nn(
            par['input_test_mod1'], model_pth, 19039, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, gene_activity=True)

        # model (14) tfidf
        model_pth = meta['resources_dir'] + '/model/weights_v4/atac2gex/model_best_AtoB_cycle_atac2gex_v2_tfidf_Nov16-23-08.pt'
