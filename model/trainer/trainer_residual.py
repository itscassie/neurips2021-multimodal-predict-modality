""" the trainer of residual method """
import os
import logging
import numpy as np
...
from modules.model_ae import CommonEncoder, ResidEncoder, ConcatEncoder


class TrainProcess:
    """the training process for residual arch"""
    ...
    def __init__(self, args):
        ...
        self.cmn_encoder = CommonEncoder(
            args.mod1_dim, args.mod2_dim, args.emb_dim, args.hid_dim
        ).to(self.device)
        self.resid_encoder = ResidEncoder(
            args.mod1_dim, args.mod2_dim, args.emb_dim, args.hid_dim
        ).to(self.device)
        self.decoder = ConcatEncoder(
            args.emb_dim, args.mod1_dim, args.mod2_dim, args.hid_dim
        ).to(self.device)
    ...
    def adjust_learning_rate(self, optimizer, epoch):
        """the adjust learning rate function"""
    ...
    def load_checkpoint(self):
        ...
        if self.args.checkpoint is not None:
            if os.path.isfile(self.args.checkpoint):
                ...
                self.resid_encoder.load_state_dict(
                    checkpoint["resid_encoder_state_dict"]
                )
    ...
    def load_pretrain(self):
        """load pre-trained model of common encoder and decoder"""
    ...
    def pretrain_epoch(self, epoch):
        """pretrain common encoder and decoder"""
        ...
        for batch_idx, (mod1_seq, mod2_seq) in enumerate(self.train_loader):
            ...
            mod1_rec, mod2_rec = self.decoder(
                mod1_cmn_emb, mod2_cmn_emb, zero_resid_emb, zero_resid_emb
            )
            ...
            cmn_emb_loss = self.args.cmn_loss_weight * self.mse_loss(
                mod1_cmn_emb, mod2_cmn_emb
            )
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (mod1_seq, mod2_seq) in enumerate(self.train_loader):
            ...
            mod1_rec, mod2_rec = self.decoder(
                mod1_cmn_emb, mod2_cmn_emb, mod1_resid_emb, mod2_resid_emb
            )
            ...
            ortho_loss = self.args.cos_loss_weight * self.cos_loss(
                mod1_cmn_emb, mod2_cmn_emb, mod1_resid_emb, mod2_resid_emb
            )
    ...
    def eval(self):
        ...
        for _, (mod1_seq, _) in enumerate(self.train_loader):
            ...
            _, mod2_rec = self.decoder(zero_emb, mod1_cmn_emb, zero_emb, zero_emb)
