...
if input_train_mod2.var['feature_types'][0] == 'ATAC':
    ...
    if LOAD_MODEL:
        ...
        # model (p2w) nn
        model_pth = meta['resources_dir'] + '/model/weights_v9/gex2atac/model_AtoB_cycle_gex2atac_p2_Nov25-04-26.pt'
        yp2w_pred_nn = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)

        # model (p4w) concat
        model_pth = meta['resources_dir'] + '/model/weights_v9/gex2atac/model_AtoB_cycle_gex2atac_p2_tfidfconcat_Nov25-13-24.pt'
        mod1_idf = np.load(meta['resources_dir'] + '/model/idf_matrix/gex2atac/mod1_idf_p2.npy')
        yp4w_pred_concat = pretrin_nn(
            par['input_test_mod1'], model_pth, MOD1_DIM*2, MOD2_DIM, FEAT_DIM, HIDDEN_DIM,
            tfidf=2, idf_matrix=mod1_idf
        )
        ...
        y_pred = (
            ...
            np.array(yp2w_pred_nn) + np.array(yp4w_pred_concat)
            ) / 6
    ...
    if LOAD_MODEL:
        ...
        # model (P5w) nn
        model_pth = meta['resources_dir'] + '/model/weights_v9/gex2adt/model_nn_gex2adt_p2_Nov26-07-09.pt'
        yP5w_pred_nn = pretrin_nn(par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM)

        # model (p10w) concat
        model_pth = meta['resources_dir'] + '/model/weights_v9/gex2adt/model_nn_gex2adt_p2_tfidfconcat_dropout0.3_Nov26-06-05.pt'
        mod1_idf = np.load(meta['resources_dir'] + '/model/idf_matrix/gex2adt/mod1_idf_v2.npy')
        yp10w_pred_concat = pretrin_nn(
            par['input_test_mod1'], model_pth, MOD1_DIM*2, MOD2_DIM, FEAT_DIM, HIDDEN_DIM,
            tfidf=2, idf_matrix=mod1_idf
        )
        ...
        y_pred = (
            ...
            np.array(yP5w_pred_nn) + np.array(yp10w_pred_concat)
        ) / 10
    ...
    if LOAD_MODEL:
        ...
        # model (p6bw) cycle
        model_pth = meta['resources_dir'] + '/model/weights_v9/adt2gex/model_AtoB_cycle_adt2gex_p2_dropout0.0_wo2_Nov26-03-45.pt'
        yp6bw_pred_nn = pretrin_nn(
            par['input_test_mod1'], model_pth, MOD1_DIM, MOD2_DIM, FEAT_DIM, HIDDEN_DIM
        )

        # model (p8bw) concat
        model_pth = meta['resources_dir'] + '/model/weights_v9/adt2gex/model_AtoB_cycle_adt2gex_p2_tfidfconcat_dropout0.0_wo2_Nov26-05-02.pt'
        mod1_idf = np.load(meta['resources_dir'] + '/model/idf_matrix/adt2gex/mod1_idf_p2.npy')
        yp8bw_pred_concat = pretrin_nn(
            par['input_test_mod1'], model_pth, MOD1_DIM*2, MOD2_DIM, FEAT_DIM, HIDDEN_DIM,
            tfidf=2, idf_matrix=mod1_idf
        )
        ...
        y_pred = (
            ...
            np.array(yp6bw_pred_nn ) + np.array(yp8bw_pred_concat)
            ) / 6
    ...
    if LOAD_MODEL:
        ...
        # model (p2bw) ga
        model_pth = meta['resources_dir'] + '/model/weights_v9/atac2gex/model_AtoB_cycle_atac2gex_p2_ga_Nov24-13-56.pt'
        yp2bw_pred_ga = pretrin_nn(
            par['input_test_mod1'], model_pth, 19039, MOD2_DIM, FEAT_DIM, HIDDEN_DIM, gene_activity=True)

        # model (p3b) 2pct
        model_pth = meta['resources_dir'] + '/model/weights_v9/atac2gex/model_best_AtoB_cycle_atac2gex_p2_select2pct_Nov24-23-16.pt'
        ...
        y_pred = (
            ...
            np.array(yp2bw_pred_ga) + np.array(yp3b_pred_2pct)
            ) / 5
